...
.gcrypt: libgcrypt
	#$(RECONF)
	cd $< && $(HOSTVARS) CC=gcc CXX=g++ ./configure $(HOSTCONF) --enable-ciphers=aes,des,rfc2268,arcfour --enable-digests=sha1,md5,rmd160,sha512 --enable-pubkey-ciphers=dsa,rsa,ecc $(CONFIGURE_OPTS)
	cd $< && $(MAKE) install
	touch $@
